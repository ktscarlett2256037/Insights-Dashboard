...
def fetch_quantum_data(symbol, horizon):
    try:
        # yahooquery uses a more direct API
        ...
        # Defensive History Fetch
        df = t.history(period=p, interval=i)
        
        if isinstance(df, str): return None, {} # Caught a string error message
        ...
        # Defensive Fundamentals Fetch
        def safe_get(prop):
            val = getattr(t, prop)
            return val.get(symbol, {}) if isinstance(val, dict) else {}

        summary = safe_get('summary_detail')
        stats = safe_get('key_stats')
...
if data is not None:
    ...
    st.info("Yahoo is currently rate-limiting this server. Wait 20 seconds and refresh.")
